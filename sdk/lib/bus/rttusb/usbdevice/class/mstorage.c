...
#if USBDISK == 1
#define RT_USE_UDISK_BACKEND (&sd_udisk_backend)       /* sd u盘 */
#elif USBDISK == 2
#define RT_USE_UDISK_BACKEND (&flash_udisk_backend)    /* flash u盘 */
#elif USBDISK == 3
#define RT_USE_UDISK_BACKEND (&sram_udisk_backend)     /* sram u盘 */
#else
#define RT_USE_UDISK_BACKEND RT_NULL
#endif
#else
...
//自定义
typedef int (*udisk_read)(void *dev, uint32_t lba, uint32_t block_count, uint8* buf);
typedef int (*udisk_write)(void *dev, uint32_t lba, uint32_t block_count, uint8* buf);
typedef uint8_t (*udisk_get_status)(void *dev);
typedef uint32_t (*udisk_get_cap)(void *dev);
typedef uint32_t (*udisk_get_sector_size)(void *dev);
struct udisk_backend_s
{
    uint16                  dev_id;
    udisk_get_sector_size   get_sector_size;    //扇区的大小
    udisk_read              read;
    udisk_write             write;
    udisk_get_status        get_status;         //这个现在只是判断是否存在
    udisk_get_cap           get_capacity;
};
//如果有sd卡,注册sd函数接口
#if SDH_EN == 1
#include "lib/sdhost/sdhost.h"

const struct udisk_backend_s sd_udisk_backend =
{
    .dev_id = HG_SDIOHOST_DEVID,
    .get_sector_size = sd_udisk_get_sector_size,
    .read = sd_udisk_read,
    .write = sd_udisk_write,
    .get_status = sd_udisk_get_status,
    .get_capacity = sd_udisk_get_capacity,
};
#endif
...
const struct udisk_backend_s flash_udisk_backend =
{
    .dev_id = 0,
    .get_sector_size = flash_udisk_get_sector_size,
    .read = flash_udisk_read,
    .write = flash_udisk_write,
    .get_status = flash_udisk_get_status,
    .get_capacity = flash_udisk_get_capacity,
};
#endif
...
static int sram_udisk_write(void *dev, uint32_t lba, uint32_t block_count, uint8* buf)
{
    hw_memcpy(usb_sram_filesystem + lba * 512, buf, block_count * 512);
    return 0;
}

static int sram_udisk_read(void *dev, uint32_t lba, uint32_t block_count, uint8* buf)
{
    hw_memcpy(buf, usb_sram_filesystem + lba * 512, block_count * 512);
    return 0;
}

static uint8_t sram_udisk_get_status(void *dev)
{
    return 0;
}

static uint32_t sram_udisk_get_capacity(void *dev)
{
    return USB_SRAM_MASS_CAPCITY / 512;
}

static uint32_t sram_udisk_get_sector_size(void *dev)
{
    return 512;
}

const struct udisk_backend_s sram_udisk_backend =
{
    .dev_id = 0,
    .get_sector_size = sram_udisk_get_sector_size,
    .read = sram_udisk_read,
    .write = sram_udisk_write,
    .get_status = sram_udisk_get_status,
    .get_capacity = sram_udisk_get_capacity,
};
...
struct mstorage
{
    ...
    const struct udisk_backend_s *backend;
    #if PINGPANG_BUF_EN
...
static rt_ssize_t _test_unit_ready(ufunction_t func, ustorage_cbw_t cbw)
{
    ...
    if(data->test_unit || data->backend->get_status(data->disk))
    {
...
static rt_ssize_t _read_capacities(ufunction_t func, ustorage_cbw_t cbw)
{
    ...
    data->geometry.sector_count = data->backend->get_capacity(data->disk);
...
#else
	rt_size_t size = 0;
    size = data->backend->read(data->disk, data->block, 1, data->ep_in->buffer);  //每个sector都进行读取sd
    if(size){
        rt_kprintf("read data error\n");
        rt_usbd_ep_set_stall(func->device, data->ep_in);
        data->csw_response.status = 1;
    ...
    case STAT_CSW:
     ...
     case STAT_SEND:
        ...
        if(data->count > 0 && data->csw_response.data_reside > 0)
        {
            ...
            read_size = data->backend->read(data->disk, data->block, 1, data->ep_in->buffer);
            if(read_size){
                rt_kprintf("disk read error\n");
                rt_usbd_ep_set_stall(func->device, data->ep_in);
                return -RT_ERROR; 
...
static rt_err_t _ep_out_handler(ufunction_t func, rt_size_t size)
{
    ...
    else if(data->status == STAT_RECEIVE)
    {
        ...
        #else
		rt_uint32_t ret = 0;
        ret = data->backend->write(data->disk, data->block, 1, data->ep_out->buffer);
        if(ret)
        {
            rt_kprintf("disk write error\n");
            goto exit;
...
static rt_err_t _function_enable(ufunction_t func)
{
    ...
    if(data->backend->dev_id != 0)
    {
        data->disk = dev_get(data->backend->dev_id);
        if(data->disk == RT_NULL)
        {
            os_printf("disk get failed, dev_id:%d\n", data->backend->dev_id);
            return -RT_ERROR;
        }
    }
    ...
    data->geometry.bytes_per_sector = data->backend->get_sector_size(data->disk);
    if(!data->backend->get_status(data->disk))
    {
        data->geometry.sector_count = data->backend->get_capacity(data->disk);
    }
    else
    ...
    usbd_mass_speed_optimize_thread_init(&data->udisk_speed_info, data->disk,
                                         data->backend->read, data->backend->write);
...
ufunction_t rt_usbd_function_mstorage_create(udevice_t device)
{
    ...
    data->backend = RT_USE_UDISK_BACKEND;
    if(data->backend == RT_NULL)
	{
        os_printf("%s :data->backend is NULL!!!",__FUNCTION__);
		return 0;
	}
