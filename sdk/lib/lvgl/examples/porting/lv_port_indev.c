...
{
    lv_indev_drv_t *indev_drv = (lv_indev_drv_t *) lv_malloc(sizeof(lv_indev_drv_t));
    if (indev_drv)
    {
...
void lv_port_touchpad_indev_init(void *user_data)
{
    ...
    indev_drv_touchpad.user_data        = user_data;
...
static void touchpad_read(lv_indev_drv_t *indev_drv, lv_indev_data_t *data)
{
    ...
    touch_multipoint_pos_t *touch_point = touch_pad_get_multipoint_xy(indev_drv->user_data);
    ...
    if (touch_point)
    {
        ...
        touch_pad_free_multipoint_xy(indev_drv->user_data, touch_point);
    }
    // touch_point 为空，说明当前这一瞬间 I2C 没读出新消息，保持 last_state
    data->point.x = last_x;
    data->point.y = last_y;
    data->state   = last_state;
    // printf("touchpad_read x:%d  y:%d  state:%d\r\n",data->point.x,data->point.y,data->state);
}
...
static void keypad_read(lv_indev_drv_t *indev_drv, lv_indev_data_t *data)
{
    lvgl_indev_s   *indev    = (lvgl_indev_s *) indev_drv->user_data;
    static uint32_t last_key = 0;
    if (indev)
    {
        /*Get whether the a key is pressed and save the pressed key*/
        uint32_t act_key = indev->read(indev);
        if (act_key != 0)
        {
            data->state = LV_INDEV_STATE_PR;
            last_key    = act_key;
