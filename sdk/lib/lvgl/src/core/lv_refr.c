...
static void draw_buf_rotate_180(lv_disp_drv_t * drv, lv_area_t * area, lv_color_t * color_p)
{
	...
	rot->CONF = 0;
	rot->CONF |= BIT(6);
	rot->AREA_CONF = (area_h<<16)|(area_w);
	rot->ADDR_FROM = (int32_t)color_p;
	rot->ADDR_TO = (int32_t)color_p;
	rot->CONF |= BIT(2);
                                                     ...
                                                     lv_color_t * orig_color_p, lv_color_t * rot_buf)
{
	...
	rot->CONF |= BIT(6);
	rot->AREA_CONF = (area_h<<16)|(area_w);
	rot->ADDR_FROM = (uint32)orig_color_p;
	rot->ADDR_TO = (int32_t)rot_buf;
    if(invert_i){
        rot->CONF |= (BIT(5) | BIT(0)) ;
    }else{
        rot->CONF |= BIT(0);
    }
...
static void draw_buf_rotate_90_sqr(bool is_270, lv_coord_t w, lv_color_t * color_p)
{
	...
	rot->CONF |= BIT(6);
	rot->AREA_CONF = (w);
	rot->ADDR_FROM = (uint32)color_p;
	rot->ADDR_TO = (uint32)color_p;
	if(is_270)
		rot->CONF |= BIT(3)|BIT(1);
	else
		rot->CONF |= BIT(1);
	while((rot->CONF&BIT(6)) == 0);
