                                                                    ...
                                                                    lv_draw_mask_radius_param_t * p)
{
	...
	SHADOW->CON0  = BIT(1);
	SHADOW->PARA0 = len;
	SHADOW->MASK_PARAM0 = (((int32)rect.x1 &0xffff))|(((int32)rect.x2&0xffff) << 16); 
	SHADOW->MASK_PARAM1 = (((int32)rect.y1 &0xffff))|(((int32)rect.y2&0xffff) << 16); 
	SHADOW->MASK_PARAM2 = (radius&0xfff)|((outer&0x01) << 16); 
	SHADOW->MASK_PARAM3 = (abs_y&0xfff)|((abs_x&0xfff) <<16);
	SHADOW->MASK_LINE   = (int32_t)mask_buf;
	SHADOW->OPA_START_ON_Y = (int32_t)(p->circle->opa_start_on_y);
	SHADOW->X_START_ON_Y = (int32_t)(p->circle->x_start_on_y);
	SHADOW->CIR_OPA      = (int32_t)(p->circle->cir_opa); 
