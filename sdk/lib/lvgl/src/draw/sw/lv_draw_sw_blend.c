                                              ...
                                              lv_coord_t dest_stride, lv_color_t color, lv_opa_t opa, const lv_opa_t * mask, lv_coord_t mask_stride)
{
    ...
    /*Masked*/
        ...
        fill->CONF &= ~(0xff<<8);
        fill->CONF |= (opa<<8);
        fill->OPA_CONF = (LV_OPA_COVER<<24)|(LV_OPA_TRANSP<<16)|(LV_OPA_MAX<<8)|(LV_OPA_MIN);
        fill->AREA = (h<<16)|(w);
        fill->DEST_ADDR      =	(int32_t)dest_buf;
        fill->MASK_ADDR      =	(int32_t)mask;
        fill->STRIDE0        = (dest_stride<<16);
        fill->STRIDE1        =	mask_stride;
        fill->SRC_ADDR       = (uint32_t)color.full;
        fill->CONF          |= BIT(2);
        while((fill->CONF&BIT(18)) == 0);
#endif
                                            ...
                                            lv_coord_t dest_stride, lv_color_t color, lv_opa_t opa, const lv_opa_t * mask, lv_coord_t mask_stride)
{
    ...
    /*Masked*/
        ...
        fill->STRIDE1        =	mask_stride;
        fill->SRC_ADDR       = (uint32_t)color.full;
        fill->FG_COLOR_SAVE  = (uint32_t)fg_color_save.full;
        fill->BG_COLOR_SAVE  = (uint32_t)bg_color_save.full;
        fill->RES_COLOR_SAVE = (uint32_t)res_color_saved.full;
        fill->OPA_SAVE       = (fg_opa_save)|(bg_opa_save<<8)|(res_opa_saved<<16);
        fill->CONF          |= BIT(3);
        while((fill->CONF&BIT(18)) == 0);

        fg_color_save.full   = fill->FG_COLOR_SAVE;
        bg_color_save.full   = fill->BG_COLOR_SAVE;
        res_color_saved.full = fill->RES_COLOR_SAVE;

        fg_opa_save   = fill->OPA_SAVE&0xff;
        bg_opa_save   = (fill->OPA_SAVE&0xff00)>>8;
        res_opa_saved = (fill->OPA_SAVE&0xff0000)>>16;
...

{
    ...
    /*Masked*/
        ...
        /*Handle opa and mask values too*/
			...
			fill->DEST_ADDR 	 =	(int32_t)dest_buf;
			fill->MASK_ADDR 	 =	(int32_t)mask;
			fill->STRIDE0		 = (dest_stride<<16)|(src_stride);
			fill->STRIDE1		 = mask_stride;
			fill->SRC_ADDR		 = (int32_t)src_buf;
			fill->CONF			|= BIT(5);
			while((fill->CONF&BIT(18)) == 0);
#endif
...

{
    ...
    /*Masked*/
        ...
        fill->FG_COLOR_SAVE  = fg_color_save.full;
        fill->BG_COLOR_SAVE  = bg_color_save.full;
        fill->RES_COLOR_SAVE = res_color_saved.full;
        fill->OPA_SAVE       = (fg_opa_save)|(bg_opa_save<<8)|(res_opa_saved<<16);
        fill->LST_DEST_COLOR = last_dest_color.full;
        fill->LST_SRC_COLOR  = last_src_color.full;
        fill->LST_RES_COLOR  = last_res_color.full;
        fill->LST_OPA        = last_opa;
        fill->CONF          |= BIT(6);
        while((fill->CONF&BIT(18)) == 0);
        ...
        last_dest_color.full = fill->LST_DEST_COLOR;
        last_src_color.full  = fill->LST_SRC_COLOR;
        last_res_color.full  = fill->LST_RES_COLOR;
        last_opa        = fill->LST_OPA;
#endif
}
