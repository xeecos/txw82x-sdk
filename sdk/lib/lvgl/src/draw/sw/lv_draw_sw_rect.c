                                                         ...
                                                         lv_coord_t r)
{
    ...
    int32_t size = sw_ori  + r;
    lv_area_t rect_hw;
	lv_draw_mask_radius_param_t * p = NULL;
    ...
    /*Set half shadow width width because blur will be repeated*/
    if(sw_ori == 1) sw = 1;
    else sw = sw_ori >> 1;//sw_pre
#endif
	sw_pos = sw;
	sw_pos += sw_ori & 1;// sw_post
	
    lv_opa_t * mask_line = lv_malloc(size);
    uint16_t * sh_ups_blur_buf = lv_malloc(size * sizeof(uint16_t));

	SHADOW->CON0  &=~BIT(1);
	SHADOW->PARA0 = ((size-1)<<16)|(size<<0);
	SHADOW->PARA1 = ((size*size - 1)<<0);
	SHADOW->PARA2 = (sw<<16)|(sw_pos);
	lv_area_copy(&rect_hw, &p->cfg.rect);
	SHADOW->MASK_PARAM0 = (((int32)rect_hw.x1 &0xffff))|(((int32)rect_hw.x2&0xffff) << 16); 
	SHADOW->MASK_PARAM1 = (((int32)rect_hw.y1 &0xffff))|(((int32)rect_hw.y2&0xffff) << 16);
	SHADOW->MASK_PARAM2 = mask_param.cfg.radius;
	SHADOW->MASK_LINE = (int32_t)mask_line;     //sram
	SHADOW->SH_BUF    = (int32_t)sh_buf;
	SHADOW->OPA_START_ON_Y =  (int32_t)(mask_param.circle->opa_start_on_y);
	SHADOW->X_START_ON_Y =  (int32_t)(mask_param.circle->x_start_on_y);
	SHADOW->CIR_OPA      =  (int32_t)(mask_param.circle->cir_opa);
	SHADOW->SH_UPS_BLUR_BUF  =  (int32_t)sh_ups_blur_buf;
	SHADOW->CON0 = BIT(0);

	while(SHADOW->CON0 & BIT(0)){	
	}
