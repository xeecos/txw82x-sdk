...
struct smtp_conn {
  ...
  _BIT_(size_supported);     /* If server supports SIZE extension according to
                              RFC 1870 */
  _BIT_(utf8_supported);     /* If server supports SMTPUTF8 extension according
                              to RFC 6531 */
  _BIT_(auth_supported);     /* AUTH capability supported by server */
};
...
struct SMTP {
  ...
  _BIT_(rcpt_had_ok);        /* Whether any of RCPT TO commands (depends on
                              total number of recipients) succeeded so far */
  _BIT_(trailing_crlf);      /* Specifies if the trailing CRLF is present */
};
...
struct cr_eob_ctx {
  ...
  _BIT_(processed_eos);  /* we read and processed an EOS */
