...
struct cf_ngtcp2_ctx {
  ...
  int qlogfd;
  _BIT_(initialized);
  _BIT_(tls_handshake_complete);       /* TLS handshake is done */
  _BIT_(use_earlydata);                /* Using 0RTT data */
  _BIT_(earlydata_accepted);           /* 0RTT was accepted by server */
...
struct h3_stream_ctx {
  ...
  _BIT_(send_closed); /* stream is local closed */
