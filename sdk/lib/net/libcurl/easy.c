 ...
 */
extern void *libcurl_mem_alloc(uint32 size);
extern void libcurl_mem_free(void *ptr);
void *libcurl_mem_calloc(size_t nmemb, size_t size)
{
    void *ptr = libcurl_mem_alloc(nmemb * size);
    if (ptr) {
        os_memset(ptr, 0, (nmemb * size));
    }
    return ptr;
}
void *libcurl_mem_realloc(void *ptr, size_t size)
{
    void *nptr = libcurl_mem_alloc(size);
    if (nptr) {
        if (ptr) {
            os_memcpy(nptr, ptr, size);
            libcurl_mem_free(ptr);
        }
    }   
    return nptr;
}
char *libcurl_mem_strdup(const char *s)
{
    size_t len;
    char *d;
...
curl_malloc_callback Curl_cmalloc = (curl_malloc_callback)libcurl_mem_alloc;
curl_free_callback Curl_cfree = (curl_free_callback)libcurl_mem_free;
curl_realloc_callback Curl_crealloc = (curl_realloc_callback)libcurl_mem_realloc;
curl_strdup_callback Curl_cstrdup = (curl_strdup_callback)libcurl_mem_strdup;
curl_calloc_callback Curl_ccalloc = (curl_calloc_callback)libcurl_mem_calloc;
...
static CURLcode global_init(long flags, bool memoryfuncs)
{
  ...
  if(memoryfuncs) {
    ...
    Curl_cmalloc = (curl_malloc_callback)libcurl_mem_alloc;
    Curl_cfree = (curl_free_callback)libcurl_mem_free;
    Curl_crealloc = (curl_realloc_callback)libcurl_mem_realloc;
    Curl_cstrdup = (curl_strdup_callback)libcurl_mem_strdup;
    Curl_ccalloc = (curl_calloc_callback)libcurl_mem_calloc;
  }
