...
struct Cookie {
  ...
  _BIT_(tailmatch);     /* tail-match the domain name */
  _BIT_(secure);        /* the 'secure' keyword was used */
  _BIT_(livecookie);    /* updated from a server, not a stored file */
  _BIT_(httponly);      /* the httponly directive is present */
  _BIT_(prefix_secure); /* secure prefix is set */
  _BIT_(prefix_host);   /* host prefix is set */
};
...
struct CookieInfo {
  ...
  _BIT_(running);    /* state info, for cookie adding information */
  _BIT_(newsession); /* new session, discard session cookies on load */
};
