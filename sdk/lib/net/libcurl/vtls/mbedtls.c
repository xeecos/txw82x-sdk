...
mbed_connect_step1(struct Curl_cfilter *cf, struct Curl_easy *data)
{
  ...
  if(ret) {
    ...
    ret = mbedtls_ssl_conf_max_frag_len(&backend->config,
                                        MBEDTLS_SSL_MAX_FRAG_LEN_4096); // 必须小于MBEDTLS_SSL_IN_CONTENT_LEN
    if(ret) {
      failf(data, "mbedtls_ssl_conf_max_frag_len returned -0x%04X", -ret);
      return CURLE_SSL_CONNECT_ERROR;
    }
#endif
