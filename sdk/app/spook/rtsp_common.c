	...
	for( ls = track->source->sess_list; ls; ls = ls->next )
	{
		...
		if( !f || ls->closed)
		{
			...
			if(!ls)
			{
				...
				if(source->release)
				{
					if(source->track_deinit)
					{
						source->track_deinit(source);
					}
...
int set_video_track( const char *name, void *d )
{
	...
	source->track[t].type = TRACK_JPEG;
	set_waiting( source->track[t].stream, 0 );
...
int set_video_h264_track( const char *name, void *d )
{
	...
	source->track[t].type = TRACK_H264;
	set_waiting( source->track[t].stream, 0 );
...
int set_audio_track( char *name, void *d )
{
	...
	source->track[t].type = TRACK_AUDIO;
	set_waiting( source->track[t].stream, 0 );
...
int del_track( struct rtsp_source *source )
{
	...
	{
		if(source->track[t].stream)
		{
			// set_waiting( source->track[t].stream, 0 );
			del_dest(source->track[t].stream);
			source->track[t].stream = NULL;
		}

		if(source->track[t].rtp)
		{	
			switch(source->track[t].type)
			{
				case TRACK_JPEG:
					del_rtp_media_jpeg_stream(source->track[t].rtp);
					break;
				case TRACK_H264:
					del_rtp_media_h264_stream(source->track[t].rtp);
					break;
				case TRACK_AUDIO:
					del_rtp_media_audio_stream(source->track[t].rtp);
					break;
				default:
					_os_printf("unknow track type: %d\r\n", source->track[t].type);
					return -1;
					break;
			}
			source->track[t].rtp = NULL;
		}
	}
	return 0;
}
...
void register_live_fn( struct rtsp_source *source,rtsp_creat creat,rtsp_release release,rtsp_play_fn play_fn, rtsp_track_deinit track_deinit )
{
	...
	source->play = play_fn;
	source->track_deinit = track_deinit;
}
