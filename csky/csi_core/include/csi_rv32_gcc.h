 ...
 */
__ALWAYS_STATIC_INLINE uint32_t __in_disable_irq(uint32_t mstatus)
{
    return !(mstatus & 0x8);
}
...
__ALWAYS_STATIC_INLINE uint32_t __in_interrupt(void)
{
    ...
    return (mcause&0x80000000) && ((mstatus&0x00001888)==0x00001880); //MPP=3,MPIE=1,MIE=0
#else
    return g_in_interrupt;
#endif
}
